...
callbacks = {None: []}
def all_callbacks(complete, event_type, position, length, num_samples, name, audio_position):
	try:
		if num_samples:
		...
		for callback in callbacks.get(current_speaker, [])[:]:
			callback(complete, wave_string, event_type, position, length, num_samples, name, audio_position)
